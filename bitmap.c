...
struct bitmap *bitmap_expand (struct bitmap *bitmap, int size){
    struct bitmap *expb = malloc (sizeof *expb);
    int newsize = bitmap->bit_cnt + size;
    int i, tmp;

    if(size < 0)    printf("Input error\n");

    if (expb != NULL){
	expb->bit_cnt = newsize;
	expb->bits = realloc (bitmap->bits, byte_cnt (newsize));
